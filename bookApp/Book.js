...
import {ResizeMixin } from "https://cdn.rawgit.com/orstavik/JoiComponents/master/src/layout/ResizeMixin.js";
import {WcChapter} from "./WcBookChapter.js";

export class WcBook extends ResizeMixin(ChildrenChangedMixin(HTMLElement)) {
  constructor() {
    super();
  ...
  render() {
    this.innerHTML = `
    <wc-chapter  title="Web components basics" folder="chapter1">
        <wc-chapter title="Pattern: CreateElement">2_HowTo_CreateElement</wc-chapter>
        <wc-chapter title="Pattern: Create a shadowDom">3_HowTo_CreateShadowDom</wc-chapter>
        <wc-chapter title="Pattern: attributeChangedCallback">7_HowTo_attributeChangedCallback</wc-chapter>
    </wc-chapter>
<!--    <wc-chapter title="Isolated Functional mixins and life cycle hooks" folder="chapter2_slot_basics">-->
<!--        <wc-chapter title="Discussion IsolatedFunctionalMixin">1_WhatIs_shadowDOM.md</wc-chapter>-->
...
<!--        &lt;!&ndash;<wc-chapter title="Pattern3 CulDeSacElements">Pattern3_CulDeSacElements</wc-chapter>&ndash;&gt;-->
...
//let subchapters links
  letLinks() {
    ...
    for (let i of chapters) {
      let link1 = [];
      let folder = i.getAttribute("folder")
      this.link.push(link1);
      for (let a of i.children) {
        link1.push(a.innerText); //folder + "/"+
      }
    }
    return this.link;
  }
